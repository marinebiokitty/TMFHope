...
\textbf{Private Communication:} Characters have no way to communicate privately 
...
The Discord server has a text channel called ``mechanics-only'' in the ``Misc.'' category. Only some roles can see this channel. It contains \textbf{only} containers for \textbf{out of character mechanics}. You may have a mechanic that tells you to go to this channel and use the ``?searchcontainer'' command to exchange one specific item for another specific item.
...
\begin{enumerate}
  \item Type into the text channel: ``Captain Jones Killing blow 1'' (Indicating who you are targeting with the mechanic.)
  \item Speak in the voice channel, and a reasonable speed and volume: ``Killing blow 2,'' ``Killing blow 3''\ldots up to ``Killing blow 9.'' 
...
\emph{Command} & \emph{Example} & \emph{Effect} & \emph{When to use}\\
  ...
  ?memtriggers & ?memtriggers & Lists the names of your memory packets, and their triggers (the conditions under which you should open them.) &  Use in your private channel.\\
    \hline,
		?triggerhere $<$memory packet name$>$ & ?trigger A packet & Opens up a memory packet. Only use this when you have met the requirements listed in the trigger. &  Use in your private channel.\\
    \hline
\end{tabularx}
