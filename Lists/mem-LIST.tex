...
\NEW{MemFold}{\mMedThree}{
  ...
  \s\MYtext	{Well, whatever these apparitions are, they aren’t human. Get a message to the Diplomat and ask them to help you investigate. **Tell them to to trigger their ``Diplomat 1'' mem packet ** (other players can carry this OOC information for you, along with a reasonable IC message) It may be a little while before they can send an answer back to you; they may not be able to see what you're seeing yet! || The Diplomat will should a message back with which page to open next.}
}
...
\NEW{MemFold}{\mDeckHandOne}{
  \s\MYname	{Code 9990| Trigger: Open if you are given a 	``Modified Walkie Talkie''.}
...
\NEW{MemFold}{\mEngineerOne}{
  \s\MYname	{Code 9990| Trigger: Open if you are given a 	``Modified Intercom Interface''.}
...
\NEW{MemFold}{\mWeaponsTwo}{
  \s\MYname	{Code 9990| Trigger: Open when told to do so by a player}
